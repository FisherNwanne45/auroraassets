						...
						<div class="col card bg-{{$bg}} shadow-lg p-4">
							@if($title=="Complete Payment")
								<div class="p-5 text-center alert-success">
									...
									<img width="220" height="220" alt="Payment QR code" src="{{$p_qrcode}}"><br><hr>
								</div>
							...
							<div class="mb-3 text-{{$text}}">
								<h4>You are to make payment of <strong>{{$settings->currency}}{{$amount}} (<?php  
...
?>  <?= $bitcoin_price ?> BTC)</strong> using your preferred mode of payment below.</h4> <br> <h2>Ensure to screenshot and submit the proof of payment below.</h2>
								<?php 
...
?>  <?= $bitcoin_price ?>	
														...
														BTC to : </strong>  {{$settings->btc_address}} <br> or scan the code to send <strong>{{$settings->currency}}{{$amount}}</strong><br><br>
														<img src="{{ asset('storage/app/public/photos/'.$settings->qrcode)}}" alt="{{$settings->site_name}}" width="200px" class="img-fluid" /><p>This is a bitcoin network address. Do not send BTC over any other network to this address or your funds may be lost. Do not send Bitcoin Cash (BCH) to this address.</p>
														
														<br/><br/>
														@if($settings->withdrawal_option != "manual")
														<a href="{{ url('dashboard/cpay') }}/{{$amount}}/BTC/{{ Auth::user()->id }}/new" class="btn btn-{{$text}}">Click to Proceed</a>
														@endif
														</h4>
													</div>
												</div>
										...
										<div class="card bg-{{$bg}} shadow text-{{$text}}">
											<div class="card-body">
												...
												<div id="ltc" class="panel-collapse collapse">
													<div class="">
														<h4 class="text-{{$text}}">
														<strong>USDT Address:</strong> {{$settings->ltc_address}}
														<br/><br/>
														@if($settings->withdrawal_option != "manual")
