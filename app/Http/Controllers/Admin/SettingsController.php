...
class SettingsController extends Controller
{
    ...
    public function updatewebinfo(Request $request){
      ...
      $this->validate($request, [
        ...
        'qrcode' => 'mimes:jpg,jpeg,png|max:500|image',
        ]);
      ...
      if($request->hasfile('qrcode'))
         {
          $qrfile = $request->file('qrcode');
          $qrname = $strtxt . $qrfile->getClientOriginalName() . time();
          // save to storage/app/uploads as the new $filename
          $pathqr = $qrfile->storeAs('public/photos', $qrname);
          
      }else {
        $qrname = $settings->qrcode;
      }
